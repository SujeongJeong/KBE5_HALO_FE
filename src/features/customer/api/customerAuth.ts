...
import type {
  CustomerSignupReq,
  CustomerSignupReqDTO
} from '../types/CustomerSignupType'
import type {
  ChargePointsType,
...
const transformToCustomerSignupReqDTO = (
  signupData: CustomerSignupReq
): CustomerSignupReqDTO => {
  return {
    userSignupReqDTO: {
...
// 수요자 포인트 충전
export const chargePoints = async (data: ChargePointsType) => {
...
// 수요자 포인트 조회
export const getCustomerPoint = async () => {
  const res = await api.get('/customers/auth/my/point')
  return res.data.body
}
