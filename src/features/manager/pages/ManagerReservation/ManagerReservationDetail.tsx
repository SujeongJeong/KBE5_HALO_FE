...
export const ManagerReservationDetail = () => {
  ...
  const [reservation, setReservation] = useState<ManagerReservationType | null>(
    null
  )
      ...
      setReservation(
        data
          ? {
              ...data,
              customerContent:
                data.customerReviewContent ?? data.customerContent,
  ...
  const handleCheck = async (checkType: 'IN' | 'OUT') => {
    try {
      ...
      const errMsg =
        ...
        `${checkType === 'IN' ? '체크인' : '체크아웃'} 요청 중 오류가 발생했습니다.`
    ...
    try {
      ...
      if (!isValidLength(content, 1, 600)) {
        setErrorToastMessage(
          '리뷰 내용은 필수 입력 항목이며, 600자 이하로 입력해주세요.'
        )
        return
      }
  ...
  return (
    <Fragment>
      <div className="inline-flex w-full flex-col items-start justify-start self-stretch">
        ...
        {/* 체크인/체크아웃 카드: Card 바깥, 본문 위에 위치 (PC/모바일 분리) */}
        ...
        <div className="flex flex-col items-start justify-start gap-6 self-stretch p-6">
          <Card className="flex flex-col items-start justify-start gap-6 self-stretch rounded-xl bg-white p-8 shadow-[0px_2px_12px_0px_rgba(0,0,0,0.04)]">
            <div className="flex w-full flex-col gap-8">
              ...
              />
              {/* 고객 정보 영역 */}
              <div className="mt-2 flex items-start gap-3 rounded-lg border border-indigo-100 bg-indigo-50 p-4">
                <svg
                  className="mt-1 h-6 w-6 text-indigo-400"
                  fill="currentColor"
                  viewBox="0 0 20 20">
                  <path
                    fillRule="evenodd"
                ...
                <div className="flex-1">
                  <div className="mb-1 text-sm font-medium text-indigo-700">
                    고객 정보
                  </div>
                  <div className="flex flex-row gap-8">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm font-medium text-gray-700">
                        {typeof (
                          customerProfile?.averageRating ??
                          reservation.averageRating
                        ) === 'number'
                          ? (
                              customerProfile?.averageRating ??
                              reservation.averageRating
      ...
      <MangerServiceCheckLogModal
        ...
        uploadedFiles={
          checkType === 'IN' ? checkInUploadedFiles : checkOutUploadedFiles
        }
        onRemoveUploadedFile={
          checkType === 'IN'
            ? handleRemoveCheckInUploadedFile
            : handleRemoveCheckOutUploadedFile
        }
