...
const ManagerNotFound = () => {
  ...
  const handleClick = () => {
    if (isLoggedIn) {
      navigate('/managers'); // 매니저 홈
    } else {
      navigate('/managers/auth/login'); // 로그인 페이지
    }
  };
    ...
    <div className="w-full min-h-screen px-6 py-10 bg-slate-50 flex flex-col justify-center items-center">
      ...
      <div className="w-full max-w-2xl p-16 bg-white rounded-2xl shadow-[0px_4px_20px_0px_rgba(0,0,0,0.04)] outline outline-1 outline-offset-[-1px] outline-zinc-100 flex flex-col justify-start items-center gap-8">
        ...
        <div className="inline-flex justify-start items-start gap-4">
          ...
          >
            <span className="text-white text-base font-semibold">홈으로 이동</span>
          </button>
          <button
            onClick={() => navigate(-1)}
          ...
          >
            <span className="text-stone-500 text-base font-medium">이전 페이지</span>
          </button>
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl mt-10 p-8 bg-white rounded-2xl shadow-[0px_4px_20px_0px_rgba(0,0,0,0.04)] outline outline-1 outline-offset-[-1px] outline-zinc-100 flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          <div className="text-zinc-800 text-xl font-bold">도움이 필요하신가요?</div>
          <div className="text-stone-500 text-sm">문제가 지속되면 고객센터로 문의해주세요.</div>
        </div>
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <div className="text-zinc-800 text-base font-semibold">평일 09:00-18:00</div>
            </div>
          </div>
        </div>
      </div>
