...
export const AdminInquiryDetail = ({ activeTab }: { activeTab: 'manager' | 'customer' }) => {
  ...
  useEffect(() => {
    ...
    getAdminInquiry(activeTab, Number(inquiryId), authorId).then((data: any) => {
      setInquiry(data);
          ...
          <div className="self-stretch p-6 flex flex-col justify-start items-start gap-5">
            <div className="self-stretch inline-flex justify-between items-center">
              <div className="flex justify-start items-center gap-2">
                <div className={`h-7 px-3 rounded-2xl flex justify-center items-center ${isAnswered ? 'bg-green-100' : 'bg-amber-100'}`}> 
                  <div className={`justify-start text-xs font-semibold font-['Inter'] leading-none ${isAnswered ? 'text-green-600' : 'text-amber-600'}`}>{answerText}</div>
                </div>
              ...
              <div className="self-stretch inline-flex justify-start items-start gap-2">
                {inquiry.fileId ? (
                  <div className="flex-1 h-11 p-3 rounded-lg outline outline-1 outline-offset-[-1px] outline-gray-200 flex justify-start items-center gap-3">
                    ...
                    <div className="flex-1 justify-start text-gray-700 text-sm font-normal font-['Inter'] leading-none">첨부파일이 있습니다</div>
