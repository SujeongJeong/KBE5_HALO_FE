...
import { creatAdminBanner, updateAdminBanner } from "@/features/admin/api/adminBanners";
...
export const AdminBannerForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && existingData) {
        await updateAdminBanner(existingData.bannerId, { title, path, startAt, endAt, fileId });
        alert("배너가 수정되었습니다.");
      ...
      <form onSubmit={handleSubmit} className="flex-1 self-stretch h-[968px] inline-flex flex-col justify-start items-start">
        ...
        <div className="self-stretch p-6 inline-flex flex-col justify-start items-start gap-6">
          <div className="self-stretch p-8 bg-white rounded-xl shadow-[0px_2px_12px_0px_rgba(0,0,0,0.04)] flex flex-col justify-start items-start gap-6">
            ...
            {/* 경로 */}
            <div className="self-stretch flex flex-col gap-2">
              <label className="text-slate-700 text-sm font-medium font-['Inter'] leading-none">경로 *</label>
              <input
                type="text"
                value={path}
            ...
            {/* 첨부파일 */}
            <FileUploadSection files={files} setFiles={setFiles} title={"배너 이미지"} multiple={false} isRequired={true}/>
