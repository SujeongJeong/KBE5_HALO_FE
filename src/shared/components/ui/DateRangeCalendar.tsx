            ...
            {days.map((date, index) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-lg p-2 text-sm transition-colors ${!isCurrentMonthDate ? "text-gray-300" : ""} ${isFutureDate(date) ? "cursor-not-allowed text-gray-400" : ""} ${isCurrentMonthDate && !isFutureDate(date) ? "text-gray-700" : ""} ${isSelected && !isFutureDate(date) ? "bg-[#6366F1] text-white" : ""} ${isInRange && !isSelected && !isFutureDate(date) ? "bg-[#6366F1]/20 text-[#6366F1]" : ""} ${isCurrentMonthDate && !isSelected && !isInRange && !isFutureDate(date) ? "hover:bg-gray-100" : ""} `}>
                  {date.getDate()}
                </button>
              )
            })}
